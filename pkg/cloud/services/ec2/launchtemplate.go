...
func (s *Service) ReconcileLaunchTemplate(
	...
	if needsUpdate || tagsChanged || amiChanged || userDataHashChanged || userDataSecretKeyChanged || launchTemplateNeedsUserDataSecretKeyTag {
		scope.Info("creating new version for launch template", "existing", launchTemplate, "incoming", scope.GetLaunchTemplate(), "needsUpdate", needsUpdate, "tagsChanged", tagsChanged, "amiChanged", amiChanged, "userDataHashChanged", userDataHashChanged, "userDataSecretChanged", userDataSecretKeyChanged)
...
func (s *Service) GetLaunchTemplate(launchTemplateName string) (*expinfrav1.AWSLaunchTemplate, string, *types.NamespacedName, error) {
...
func (s *Service) CreateLaunchTemplate(scope scope.LaunchTemplateScope, imageID *string, userDataSecretKey types.NamespacedName, userData []byte) (string, error) {
...
func (s *Service) CreateLaunchTemplateVersion(id string, scope scope.LaunchTemplateScope, imageID *string, userDataSecretKey types.NamespacedName, userData []byte) error {
...
func (s *Service) createLaunchTemplateData(scope scope.LaunchTemplateScope, imageID *string, userDataSecretKey types.NamespacedName, userData []byte) (*ec2.RequestLaunchTemplateData, error) {
...
func (s *Service) SDKToLaunchTemplate(d *ec2.LaunchTemplateVersion) (*expinfrav1.AWSLaunchTemplate, string, *types.NamespacedName, error) {
	...
	var launchTemplateUserDataSecretKey *types.NamespacedName
		...
		if tagSpecification.ResourceType != nil && *tagSpecification.ResourceType == ec2.ResourceTypeInstance {
			for _, tag := range tagSpecification.Tags {
				if tag.Key != nil && *tag.Key == infrav1.LaunchTemplateBootstrapDataSecret && tag.Value != nil && strings.Contains(*tag.Value, "/") {
					...
					launchTemplateUserDataSecretKey = &types.NamespacedName{
						Namespace: parts[0],
						Name:      parts[1],
					}
				}
...
func (s *Service) buildLaunchTemplateTagSpecificationRequest(scope scope.LaunchTemplateScope, userDataSecretKey types.NamespacedName) []*ec2.LaunchTemplateTagSpecificationRequest {
