...
func (s *Service) reconcileNatGateways() error {
	...
	if len(s.scope.Subnets().FilterPrivate().FilterPrimary()) == 0 {
		...
		return nil
	} else if len(s.scope.Subnets().FilterPublic().FilterPrimary()) == 0 {
	...
	for _, sn := range s.scope.Subnets().FilterPublic().FilterPrimary() {
		if sn.GetResourceID() == "" {
			continue
		}
